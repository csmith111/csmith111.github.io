...
If for example we want to create a program to count the mouse clicks that have happened so far, then we need to keep a running count of the clicks that have happened it and then increment it every time a new click happens.
To functional programers there is a similar problem with lists.
How do you sum the elements for a list when you have to take an initial value and then increment it. This is done using a fold.
You can see an example of this in the post on [Elm basics][Elm Basics].
There is a a function similar to fold that works for signals called **foldp** (for fold over the past?)
Looking at the signature:

`foldp : (a -> s -> s) -> s -> Signal a -> Signal s`

we see that `foldp` takes a function and a generic initial state type and threads this state type through the signal, keeping and accumulating state.
